let active: string = '1-1'; // 현재 활성화 되어 있는 Board의 Season
let lang: string = 'kr'; // 언어 사전 설정

const gpa_tab: NodeListOf<HTMLElement> = document.querySelectorAll('.gpa-tab'); // Tab elements
...
function changeBoardTo(season :string) { // Board를 변경하는 함수
    $.getJSON('../json/' + season + '.json', function (data) { // Season => (ex. 1-1, 1-2) 에 해당하는 Json 파일 로드
        ...
        let html :string = ''; // Table HTML stack
